...
int main(){
    printf("Лаб.раб.5.3.Выполнила студентка группы ИСТбд-21 Ахвандеева О.С.");
    int cpp = open("/home/student/new/5/main.c", O_RDONLY);
    struct stat st;
    lstat("/home/student/new/5/main.c", &st);//lstat получает информацию об атрибутах файла
    int s = st.st_size;
    key_t key = ftok("/home/student/new/5", 1);//генерируем ключ
    int sh;
