...
/*Задание: В разделяемой памяти размещается массив из трех целых чисел.
Первый элемент массива используется как счетчик для программы 1, второй элемент – для
программы 2, третий элемент – для обеих программ суммарно.*/
///Выполнил студент группы ИСТбд-22 Каяшов П.А.
int main()
{
printf("Лабораторная работа №5.1.1\n");
...
key_t key; ///IPC ключ
///Генерируем IPC ключ из имени файла 1.c в текущей директории и номера экземпляра области
